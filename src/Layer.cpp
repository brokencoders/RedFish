...
namespace RedFish {
    ...
    bool Layer::training = true;

    void Layer::useOptimizer(Optimizer &optimizer)
    {
    ...
    Layer* make_layer(const Layer::Descriptor& dsc)
    {
        ...
        case LAYER::LINEAR:
            ...
            return new LinearLayer(dsc.param[0]._size_t, dsc.param[1]._size_t);
        case LAYER::RECURRENT_RELU:
            ...
            return new RecurrentLayer<Activation::ReLU>(dsc.param[0]._size_t, dsc.param[1]._size_t);
        case LAYER::RECURRENT_TANH:
            ...
            return new RecurrentLayer<Activation::TanH>(dsc.param[0]._size_t, dsc.param[1]._size_t);

        case LAYER::CONV1D:
            if (dsc.param.size() < 6) throw std::length_error("Not enouth parameters for Conv1dLayer construction");
            return new Conv1dLayer(dsc.param[0]._size_t, dsc.param[1]._size_t, dsc.param[2]._size_t,
        ...
        case LAYER::CONV2D:
            ...
            return new Conv2dLayer(dsc.param[0]._size_t, dsc.param[1]._size_t, dsc.param[2]._size_t,
        ...
        case LAYER::CONV3D:
            ...
            return new Conv3dLayer(dsc.param[0]._size_t, dsc.param[1]._size_t, dsc.param[2]._size_t,
    ...
    Layer *make_layer(std::ifstream &file)
    {
        ...
        if (layer_name == "Activation::PReLU")     return new Activation::PReLU(file);
        ...
        if (layer_name == "Conv1d")                return new Conv1dLayer(file);
        if (layer_name == "Conv2d")                return new Conv2dLayer(file);
        if (layer_name == "Conv3d")                return new Conv3dLayer(file);
        if (layer_name == "Dropout")               return new DropoutLayer(file);
        if (layer_name == "Flatten")               return new FlattenLayer(file);
        if (layer_name == "Linear")                return new LinearLayer(file);
